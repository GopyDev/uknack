...
class KnackManager(models.Manager):
    def get_queryset(self):
        return super(KnackManager, self).get_queryset().filter(deleted=False).order_by('-reposted_at')
...
class Knack(models.Model):
    ...
    reposted_at = models.DateTimeField(auto_now_add=True, null=True)
    ...
    @property
    def ready_to_repost(self):
        if not self.reposted_at:
            self.reposted_at = timezone.now()
        delta = timezone.now() - self.reposted_at
        if delta.days > 0:
            return True
    ...
    @property
    def repost_available_in(self):
        if self.ready_to_repost:
            return 0

        delta = timezone.now() - self.reposted_at
        return 60 * 60 * 24 - delta.seconds

    def repost(self, save=True):
        self.reposted_at = timezone.now()
        if save:
            self.save()
