...
class KnackRepostView(APIView):
    def post(self, request, pk):
        try:
            knack = Knack.objects.get(pk=pk)
        except ObjectDoesNotExist:
            return Response({}, status=status.HTTP_404_NOT_FOUND)

        if knack.owner != request.user:
            return Response({}, status=status.HTTP_403_FORBIDDEN)

        if knack.ready_to_repost:
            knack.repost()
            return Response({'success': True, 'available_in': knack.repost_available_in}, status=status.HTTP_200_OK)

        return Response({'success': False, 'available_in': knack.repost_available_in}, status=status.HTTP_200_OK)
