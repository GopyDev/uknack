...
define(['app'], function (app) {
    app.register.controller('knacksCtrl', [
        'localStorageService', '$rootScope', '$scope', '$resource', '$modal', '$state', '$stateParams', '$window', 'user',
        function (localStorageService, $rootScope, $scope, $resource, $modal, $state ,$stateParams, $window, user) {
                ...
                if ($scope.current_type != 'knack-ideas') {
                    var knacks_resource = $resource(
                        ...
                        "gender=:gender&min_price=:min_price&max_price=:max_price&min_age=:min_age&max_age=:max_age&categories=:categories&college=:college&year=:year&search_text=:search_text",
                        {
                            ...
                            connections_only: $scope.selectedItem == 'My Connections' ? user.profile().id : 0
                        }
                    );
                } else {
            ...
            $scope.flipAll = function() {
                if($rootScope.is_authenticated) {
                    ...
                    var modalInstance = $modal.open({
                        ...
                        resolve: {
                            message: function () {
                                var message = 'Please login to connect with friends and classmates';
                                return message;
                            }
                        }
                    });
