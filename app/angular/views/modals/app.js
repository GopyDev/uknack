...
define(['angularAMD'], function(app) {
    app.controller('KnackModalCtl',
        ['$scope', '$rootScope', '$resource', '$modalInstance', '$http', '$state', '$modal', 'knack', 'type',
            function ($scope, $rootScope, $resource, $modalInstance, $http, $state, $modal, knack, type) {
                $scope.knack = {
                    anonymous: false,
                    username: '',
                    name: '',
                ...
                if (knack) {
                    $scope.knack = angular.copy(knack);
                ...
                $scope.save_knack = function () {
                    ...
                    if ($scope.postForm.$valid) {
                        ...
                        if ($scope.create || $scope.type=='repost') {
                            // Add new
    ...
    app.controller('NewKnackModalCtl',
        ['$scope', '$rootScope', '$resource', '$modalInstance', '$http', '$state', '$modal', 'knack',
            function ($scope, $rootScope, $resource, $modalInstance, $http, $state, $modal, knack) {
                $scope.cancel = function () {
                    $modalInstance.dismiss('cancel');
                };
        ...
        ['$scope', '$rootScope', '$resource', '$modalInstance', '$http', '$state', '$modal', 'item', '$stateParams',
            ...
            function ($scope, $rootScope, $resource, $modalInstance, $http, $state, $modal, item, $stateParams) {
                ...
                if (item) {
                    ...
                    $scope.item_type = item.type == 'O' ? 'Offered' : 'Wanted';
                    $scope.category_name = $scope.item.category_name;
                } else {
                    $scope.item_type = 'Offered';
                    $scope.category_name = 'Category Selection';
                    if ($state.current.name == 'item-offered') {
                        ...
                        $scope.item.type = 'W';
                        $scope.item_type = 'Wanted';
                    }
                }

                $scope.create = ! $scope.item.id;
                $scope.clearUpload = function(){
                    $scope.$apply(function () {
                        $scope.cropperImages[$scope.selectedImageIndex] = null;
                    });
    ...
    app.controller('NewItemModalCtl',
        ['$scope', '$rootScope', '$resource', '$modalInstance', '$http', '$state', '$modal', 'item',
            function ($scope, $rootScope, $resource, $modalInstance, $http, $state, $modal, item) {
                $scope.cancel = function () {
                    $modalInstance.dismiss('cancel');
                };
        ...
        app.controller('RegisterEmailModalCtl',
            ...
            $scope.register = function () {
                ...
                $scope.email = RegisterEmail.save($scope.user, function(data){
					...
					$scope.message = 'An account is already associated with this email';
				});
   ...
   app.controller('LoginToFlipModalCtl',
            ['$scope', '$rootScope', '$resource', '$modalInstance', '$modal', 'message',
                '$http', 'localStorageService', 'rest', '$state', 'restricted',
            function ($scope, $rootScope, $resource, $modalInstance, $modal, message, $http, localStorageService, $state) {
